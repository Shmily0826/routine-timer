...
interface Stats { count:number; rounds:number; duration:string; }
...
Page({
  ...
  load(){
    try{
      ...
      const sec=this.records.reduce((a:number,r:HistoryRecord)=>a+(r.totalWorkSec||0)+(r.totalRestSec||0),0);
    ...
    }catch{this.records=[];this.setData({items:[],stats:null})}
  },
  clear(){
