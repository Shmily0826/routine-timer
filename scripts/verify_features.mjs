...
try {
  ...
  check(
    'history page has stats field',
    histData && 'stats' in histData,
    'keys=' + Object.keys(histData || {}).join(','),
  );
  check(
    'stats.sessions = 3',
    histData && histData.stats && histData.stats.sessions === 3,
    JSON.stringify(histData && histData.stats),
  );
  check(
    'stats.totalRounds = 10',
    histData && histData.stats && histData.stats.totalRounds === 10,
    JSON.stringify(histData && histData.stats),
  );
  // totalSec = 90+20 + 150+30 + 60+10 = 360
  check(
    'stats.totalSec = 360',
    histData && histData.stats && histData.stats.totalSec === 360,
    JSON.stringify(histData && histData.stats),
  );
  // 最近 7 天：a(1天前), b(2天前), c(7天前=168h, 应在7天边缘)
  // 计算: c 在 7*86400s 前，严格 ≤ 7d，应计 3 次（实现见源码用的是 ts/86400000 对比 now）
  check(
    'stats.recentCount = 3 (within 7 days)',
    histData && histData.stats && histData.stats.recentCount === 3,
    JSON.stringify(histData && histData.stats),
  );
  check(
    'history items rendered = 3',
    histData && histData.items && histData.items.length === 3,
    'items=' + (histData && histData.items && histData.items.length),
  );
  ...
  const histShot = await mp.evaluate(() => {
    const h = getCurrentPages().find((p) => (p.route || p.__route__) === 'pages/history/history');
    return h
      ? { hasStats: !!h.data.stats, stats: h.data.stats, itemCount: h.data.items.length }
      : null;
  });
