...
try {
  ...
  check(
    'zero-rest: starts on round 1 / work',
    s && s.phase === 'work' && s.idx === 0,
    JSON.stringify(s),
  );
  ...
  s = await timerState();
  check(
    'zero-rest: 0s rest skipped -> round 2 / work (never parked in rest)',
    s && s.idx === 1 && s.phase === 'work',
    JSON.stringify(s),
  );
  check(
    'zero-rest: session still running (not completed early)',
    s && s.status === 'running',
    JSON.stringify(s),
  );
  ...
  s = await timerState();
  check(
    'completion: both 1s rounds finish -> status completed',
  ...
  check(
    'completion: exactly one history record written',
    Array.isArray(hist) && hist.length === 1,
    JSON.stringify(hist && hist.length),
  );
  check(
    'completion: record fields (rounds=2, work=2s, rest=0s, label=甲)',
    hist[0] &&
      hist[0].rounds === 2 &&
      hist[0].totalWorkSec === 2 &&
      hist[0].totalRestSec === 0 &&
      hist[0].label === '甲',
  ...
  hist = await history();
  check(
    'completion: repeated ticks do not duplicate the record',
    Array.isArray(hist) && hist.length === 1,
    'len=' + (hist && hist.length),
  );
  ...
  hist = await history();
  check(
    'cap: history stays at 100 records after a new completion',
    Array.isArray(hist) && hist.length === 100,
    'len=' + (hist && hist.length),
  );
  check(
    'cap: newest record is stored first (label=新)',
    hist && hist[0] && hist[0].label === '新',
    JSON.stringify(hist && hist[0]),
  );
  check(
    'cap: oldest record dropped (old99 gone)',
    hist && !hist.some((r) => r.id === 'old99'),
    'old99 still present',
  );
  check(
    'cap: earlier records kept (old0 present)',
    hist && hist.some((r) => r.id === 'old0'),
    'old0 missing',
  );
